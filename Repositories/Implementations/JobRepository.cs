...
namespace Skillora.Repositories.Implementations
{
    ...
    public class JobRepository : IJobRepository
    {
        ...
        {
            return _appDbContext.Jobs.Include(j=>j.Company).Include(j => j.SkillJobs).ThenInclude(sj => sj.Skill).Include(j => j.JobConstraint).Include(j => j.StudentJobs).ThenInclude(sj => sj.Student).ThenInclude(s => s.SkillStudents).ThenInclude(s => s.Skill).Include(s=>s.SelectedStudentJobs).ThenInclude(ssj=>ssj.Student).ThenInclude(s=>s.SkillStudents).ThenInclude(s=>s.Skill).FirstOrDefault(j => j.Id == id);
        }
